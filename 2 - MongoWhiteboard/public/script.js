...
function saveimg() {
    //construct name of the file with date infos
    ...
    date = `${hour}h${min}min${sec}s ${day}-${month}-${year}`;

    //returned infos to the uploaded images endpoint
    const infos = {
        'user': username,
        'date': date,
        'url': dataURL
    ...
    fetch("/uploaded_images", fetchOptions);
    //display to the user that his image is saved
    alert('Your image is saved !');
}

// SAVED IMAGES ON MONGODB - NOT AVAILABLE YET

function savedimg() {
    alert('This function is not available yet. Your canvas are only saved on your computer for now !');
}
